...
function ComplaintPage({onBack}){
  ...
  const[submitted,setSubmitted]=useState(false);const[pinningLocation,setPinningLocation]=useState(false);
  ...
  useEffect(()=>{
    if(!pinningLocation||!locMapRef.current||locMapInstanceRef.current)return;if(!window.L)return;
    const L=window.L;const map=L.map(locMapRef.current,{center:[13.0827,80.2707],zoom:13,zoomControl:false});
    ...
    locMapInstanceRef.current=map;return()=>{map.remove();locMapInstanceRef.current=null;locMarkerRef.current=null;};
  },[pinningLocation]);

  const handleFileUpload=(e)=>{Array.from(e.target.files).forEach(file=>{const reader=new FileReader();reader.onload=(ev)=>{setForm(f=>({...f,evidence:[...f.evidence,{name:file.name,type:file.type,preview:ev.target.result,size:file.size}]}));};reader.readAsDataURL(file);});};
  const removeEvidence=(idx)=>{setForm(f=>({...f,evidence:f.evidence.filter((_,i)=>i!==idx)}));};
  const handleSubmit=()=>{if(!form.type||!pinnedLocation)return;if(form.type==='Other'&&!form.otherDesc)return;setSubmitted(true);setTimeout(()=>{setSubmitted(false);setForm({type:'',description:'',otherDesc:'',date:'',time:'',evidence:[]});setPinnedLocation(null);setPinningLocation(false);},3000);};

  const incidentTypes=['Harassment','Stalking','Theft / Robbery','Assault','Eve-Teasing','Unsafe Area','Poor Lighting','Other'];
  const canSubmit=form.type&&pinnedLocation&&(form.type!=='Other'||form.otherDesc);
  ...
  if(submitted)return(<div style={{height:'100%',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'#07070e',animation:'scaleIn 0.4s ease',fontFamily:"'Poppins',sans-serif"}}><div style={{width:72,height:72,borderRadius:'50%',background:'#34d39915',border:'2px solid #34d399',display:'flex',alignItems:'center',justifyContent:'center',fontSize:32,marginBottom:20,animation:'checkPop 0.5s cubic-bezier(0.34,1.4,0.64,1)'}}>✓</div><div style={{color:'#34d399',fontSize:17,fontWeight:800,marginBottom:6}}>Report Submitted</div><div style={{color:'#ffffff33',fontSize:12}}>Thank you for keeping everyone safe</div></div>);
  ...
  if(pinningLocation)return(
    <div style={{height:'100%',display:'flex',flexDirection:'column',background:'#07070e'}}>
      <div style={{padding:'16px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0,borderBottom:'1px solid #ffffff08'}}>
        <button onClick={()=>setPinningLocation(false)} style={{background:'#ffffff08',border:'none',borderRadius:10,padding:'8px 14px',color:'#fff',fontSize:13,fontWeight:600,fontFamily:"'Poppins',sans-serif",cursor:'pointer'}}>← Back</button>
        <span style={{fontSize:11,fontWeight:700,color:'#e81850',letterSpacing:'0.12em',fontFamily:"'Poppins',sans-serif"}}>PIN LOCATION</span>
        {pinnedLocation?<button onClick={()=>setPinningLocation(false)} style={{background:'#e81850',border:'none',borderRadius:10,padding:'8px 16px',color:'#fff',fontSize:12,fontWeight:700,fontFamily:"'Poppins',sans-serif",cursor:'pointer'}}>Done</button>:<div style={{width:60}}/>}
      </div>
      <div style={{flex:1,position:'relative'}}>
        <div ref={locMapRef} style={{height:'100%',width:'100%'}}/>
        {!pinnedLocation&&<div style={{position:'absolute',bottom:20,left:'50%',transform:'translateX(-50%)',background:'rgba(7,7,14,0.92)',border:'1px solid #e8185028',color:'#fff',padding:'10px 24px',borderRadius:30,fontSize:12,letterSpacing:'0.06em',fontFamily:"'DM Mono',monospace",pointerEvents:'none',animation:'breathe 2.4s ease-in-out infinite'}}>tap to pin incident location</div>}
        {pinnedLocation&&<div style={{position:'absolute',bottom:20,left:'50%',transform:'translateX(-50%)',background:'rgba(7,7,14,0.92)',border:'1px solid #34d39933',color:'#34d399',padding:'8px 20px',borderRadius:20,fontSize:11,fontFamily:"'DM Mono',monospace"}}>📍 {pinnedLocation.lat}, {pinnedLocation.lng}</div>}
      </div>
    </div>
  );

  return(
    <div style={{height:'100%',display:'flex',flexDirection:'column',background:'#07070e',fontFamily:"'Poppins',sans-serif"}}>
      <div style={{padding:'16px 20px 14px',flexShrink:0,borderBottom:'1px solid #ffffff08'}}>
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          <button onClick={onBack} style={{background:'#ffffff08',border:'none',borderRadius:10,padding:'8px 14px',color:'#fff',fontSize:13,fontWeight:600,fontFamily:"'Poppins',sans-serif",cursor:'pointer'}}>←</button>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'16px 20px 24px'}}>
        <div style={{marginBottom:18}}><label style={lbl}>WHAT HAPPENED?</label>
          <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
            {incidentTypes.map(t=>{const active=form.type===t;return(<button key={t} onClick={()=>setForm(f=>({...f,type:t}))} style={{padding:'10px 16px',borderRadius:20,background:active?'#e8185018':'#0d0d14',border:`1px solid ${active?'#e81850':'#ffffff0c'}`,color:active?'#e81850':'#ffffff55',fontSize:12,fontWeight:600,fontFamily:"'Poppins',sans-serif",cursor:'pointer',transition:'all 0.15s'}}>{t}</button>);})}
          </div>
        </div>
        ...
        <div style={{marginBottom:18}}><label style={lbl}>INCIDENT LOCATION</label>
          <button onClick={()=>setPinningLocation(true)} style={{width:'100%',padding:'14px 16px',background:pinnedLocation?'#0d1410':'#0d0d14',border:`1px solid ${pinnedLocation?'#34d39933':'#ffffff0c'}`,borderRadius:12,cursor:'pointer',display:'flex',alignItems:'center',gap:10,transition:'all 0.2s'}}>
            <span style={{fontSize:20}}>{pinnedLocation?'📍':'🗺️'}</span>
            <div style={{flex:1,textAlign:'left'}}>{pinnedLocation?<><div style={{fontSize:12,color:'#34d399',fontWeight:600,fontFamily:"'Poppins',sans-serif"}}>Location Pinned</div><div style={{fontSize:10,color:'#ffffff33',marginTop:2,fontFamily:"'DM Mono',monospace"}}>{pinnedLocation.lat}, {pinnedLocation.lng}</div></>:<div style={{fontSize:13,color:'#ffffff44',fontFamily:"'Poppins',sans-serif"}}>Tap to pin location on map</div>}</div>
            <span style={{fontSize:14,color:'#ffffff22'}}>→</span>
          </button>
        </div>
        ...
        <div style={{marginBottom:24}}><label style={lbl}>EVIDENCE</label>
          ...
          <button onClick={()=>fileInputRef.current?.click()} style={{width:'100%',padding:'20px 16px',background:'#0d0d14',border:'2px dashed #ffffff12',borderRadius:14,cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:8,transition:'border-color 0.2s'}}>
            ...
            <span style={{fontSize:12,color:'#ffffff44',fontFamily:"'Poppins',sans-serif",fontWeight:500}}>Upload photos, videos, or audio</span>
            <span style={{fontSize:10,color:'#ffffff1a',fontFamily:"'Poppins',sans-serif"}}>Tap to browse files</span>
          </button>
          {form.evidence.length>0&&<div style={{display:'flex',gap:8,marginTop:10,overflowX:'auto',padding:'4px 0'}}>
            {form.evidence.map((ev,i)=><div key={i} style={{position:'relative',flexShrink:0}}>
              {ev.type.startsWith('image')?<img src={ev.preview} style={{width:64,height:64,borderRadius:10,objectFit:'cover',border:'1px solid #ffffff0f'}}/>:
              <div style={{width:64,height:64,borderRadius:10,background:'#0d0d14',border:'1px solid #ffffff0f',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:2}}>
                <span style={{fontSize:18}}>{ev.type.startsWith('video')?'🎬':'🎤'}</span>
                <span style={{fontSize:7,color:'#ffffff33',maxWidth:56,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontFamily:"'Poppins',sans-serif"}}>{ev.name}</span>
              </div>}
              <button onClick={()=>removeEvidence(i)} style={{position:'absolute',top:-4,right:-4,width:18,height:18,borderRadius:'50%',background:'#e81850',border:'none',color:'#fff',fontSize:9,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Poppins',sans-serif",fontWeight:700}}>✕</button>
            </div>)}
          </div>}
        </div>
        <button onClick={handleSubmit} disabled={!canSubmit} style={{width:'100%',padding:'15px',background:canSubmit?'#e81850':'#1a0c14',border:'none',borderRadius:14,color:canSubmit?'#fff':'#ffffff1a',fontSize:14,fontWeight:700,letterSpacing:'0.08em',fontFamily:"'Poppins',sans-serif",cursor:canSubmit?'pointer':'not-allowed',boxShadow:canSubmit?'0 0 30px #e8185033':'none',transition:'all 0.3s'}}>SUBMIT REPORT</button>
      </div>
    </div>
  );
}
...
function LeafletMap({pin,onMapClick,nearby,safePlaces,dest,routeData,navigating,stepIndex,showUnsafe,unsafeZones}){
  ...
  useEffect(()=>{const map=mRef.current;if(!map)return;const L=window.L;
    ...
    if(showUnsafe&&unsafeZones)unsafeZones.forEach(z=>{const c=riskColor(z.risk),isN=z.source==='newsapi';layRef.current.push(L.circle([z.lat,z.lng],{radius:z.radius,color:c,fillColor:c,fillOpacity:isN?.10:.07,weight:isN?1.5:1,opacity:isN?.50:.35,dashArray:isN?'6 4':'4 4'}).addTo(map),L.circle([z.lat,z.lng],{radius:z.radius*.45,color:c,fillColor:c,fillOpacity:isN?.18:.14,weight:0}).addTo(map));const s=isN?12:10;const m=L.marker([z.lat,z.lng],{icon:L.divIcon({className:'',html:`<div style="width:${s}px;height:${s}px;background:${c};border-radius:50%;opacity:0.85;border:2px solid ${isN?'#ffffffaa':'rgba(255,255,255,0.25)'};box-shadow:0 0 ${isN?12:8}px ${c}cc;${isN?'animation:newsPulse 2s ease-in-out infinite;':''}"></div>`,iconSize:[s,s],iconAnchor:[s/2,s/2]})}).addTo(map);m.bindPopup(`<div style="background:#0d060a;border:1px solid ${c}44;color:#fff;padding:8px 12px;border-radius:8px;font-size:11px;font-family:'DM Mono',monospace;line-height:1.6;min-width:160px;max-width:240px"><div style="color:${c};font-weight:600;margin-bottom:4px">${isN?'📰':'⚠'} ${z.label}</div><div style="color:#ffffff66;font-size:10px">${z.note}</div><div style="margin-top:6px;font-size:9px;padding:2px 6px;border-radius:3px;background:${c}22;color:${c};border:1px solid ${c}44;display:inline-block">${Math.round(z.risk*100)}% RISK</div></div>`,{className:'st-popup'});layRef.current.push(m);});
...
function MapPage({onSOS,onComplaint,onMap,activeTab,showSOS,sosAnim}){
  ...
  function handleReset(){setPin(null);setNearby([]);setDest(null);setRouteData(null);setNavigating(false);setStepIndex(0);setLoading(false);}
  ...
  return(
    <div style={{height:'100%',width:'100%',position:'relative',background:'#07070e'}}>
      ...
      {/* ── Quick-stat cards (before pin drop) ────────────────── */}
      {!pin&&mapReady&&!navigating&&(
        <div style={{position:'absolute',bottom:90,left:0,right:0,zIndex:800,padding:'0 16px',pointerEvents:'none',animation:'fadeUp 0.5s ease both',animationDelay:'0.2s'}}>
          <div style={{display:'flex',gap:10,marginBottom:14}}>
            {[
              {icon:'🛡️',label:'Safe Spots',value:`${safeCount}`,sub:'Police & Hospitals',color:'#34d399'},
              {icon:'⚠️',label:'Risk Zones',value:`${riskTotal}`,sub:`${riskHigh} high risk`,color:'#ff2d55'},
              {icon:'📡',label:'Status',value:'Active',sub:'Monitoring area',color:'#38bdf8'},
            ].map((card,i)=>(
              <div key={i} style={{
                flex:1,background:'rgba(7,7,14,0.92)',backdropFilter:'blur(16px)',
                border:'1px solid #ffffff08',borderRadius:16,padding:'14px 12px',
                pointerEvents:'all',
                animation:`fadeUp 0.4s ease both`,animationDelay:`${0.3+i*0.1}s`,
              }}>
                <div style={{fontSize:16,marginBottom:6}}>{card.icon}</div>
                <div style={{fontSize:18,fontWeight:700,color:card.color,fontFamily:"'Poppins',sans-serif",lineHeight:1}}>{card.value}</div>
                <div style={{fontSize:9,color:'#ffffff44',fontFamily:"'Poppins',sans-serif",fontWeight:500,marginTop:3}}>{card.sub}</div>
              </div>
            ))}
          </div>
          <div style={{
            background:'rgba(7,7,14,0.9)',border:'1px solid #e8185020',
            borderRadius:24,padding:'12px 24px',
            textAlign:'center',pointerEvents:'none',
            animation:'breathe 2.4s ease-in-out infinite',
          }}>
            <span style={{fontSize:13,color:'#fff',fontFamily:"'Poppins',sans-serif",fontWeight:500}}>Tap anywhere to drop your pin</span>
          </div>
        </div>
      )}
      ...
      {/* Nav overlay */}
      {navigating&&routeData&&<NavOverlay steps={routeData.steps} stepIndex={stepIndex} setStepIndex={setStepIndex} totalDist={routeData.totalDist} totalTime={routeData.totalTime} destName={dest?.name} onEnd={handleReset}/>}
