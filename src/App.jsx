...
// ═══════════════════════════════════════════════════════════════════
function MapPage({ onSOS, onComplaint, onMap, onTrack, activeTab, sosAnim }) {
    ...
    <div style={{ height: '100%', width: '100%', position: 'relative', background: '#000000' }}>
      ...
      {!navigating && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 800, padding: '14px 20px 10px', paddingRight: '72px', background: 'linear-gradient(180deg,rgba(0,0,0,0.96) 0%,rgba(0,0,0,0.7) 60%,transparent 100%)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontFamily: "'Poppins',sans-serif" }}>
          <span style={{ fontSize: 28, fontWeight: 800, color: '#fff', letterSpacing: '-0.01em' }}>Safe</span><span style={{ fontSize: 28, fontWeight: 800, color: '#e81850', letterSpacing: '-0.01em' }}>Trace</span>
        </div>
...
export default function App() {
  ...
  return (<PhoneFrame><Styles />
    ...
    {page === 'home' && <MapPage activeTab="map" onSOS={triggerSOS} onComplaint={() => setPage('complaint')} onMap={() => { }} onTrack={() => setPage('track')} sosAnim={sosAnim} />}
    ...
    {page !== 'landing' && (
      <>
        ...
        {showSOS && <SOSOverlay onClose={triggerSOS} />}
      </>
    )}
  </PhoneFrame>);
}
