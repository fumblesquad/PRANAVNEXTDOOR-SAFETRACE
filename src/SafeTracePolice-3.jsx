...
// ── Shared store ─────────────────────────────────────────────────────
const STORE_KEY = 'safetrace_complaints_v1';
const BC_NAME   = 'safetrace_sync';

function readStore()   { try { return JSON.parse(localStorage.getItem(STORE_KEY) || '[]'); } catch { return []; } }
function writeStore(d) {
  localStorage.setItem(STORE_KEY, JSON.stringify(d));
  try { const bc = new BroadcastChannel(BC_NAME); bc.postMessage({ type: 'update' }); bc.close(); } catch {}
}

const DEMO = [
  { id:'ST-2901', type:'Harassment', location:'Anna Nagar Bus Stop', date:'2026-03-30', time:'21:45', description:'A man was persistently following a woman from the platform. He refused to leave after she asked him to stop multiple times.', severity:'high', status:'submitted', statusHistory:[{status:'submitted',ts:'30 Mar 2026, 9:47 PM',note:''}], submittedAt:'30 Mar 2026, 9:47 PM' },
  { id:'ST-2902', type:'Street Lighting', location:'Velachery Flyover Underpass', date:'2026-03-30', time:'22:10', description:'Complete absence of street lighting under the flyover. Extremely unsafe for pedestrians after 9 PM.', severity:'medium', status:'reviewing', statusHistory:[{status:'submitted',ts:'30 Mar 2026, 10:12 PM',note:''},{status:'reviewing',ts:'30 Mar 2026, 10:31 PM',note:'Assigned to patrol officer.'}], submittedAt:'30 Mar 2026, 10:12 PM' },
  { id:'ST-2903', type:'Suspicious Activity', location:'Nungambakkam Back Lane', date:'2026-03-29', time:'23:00', description:'Group of 4-5 men loitering near ATM, repeatedly approaching lone individuals asking for money.', severity:'high', status:'escalated', statusHistory:[{status:'submitted',ts:'29 Mar 2026, 11:05 PM',note:''},{status:'reviewing',ts:'29 Mar 2026, 11:22 PM',note:''},{status:'escalated',ts:'30 Mar 2026, 12:01 AM',note:'Escalated to duty inspector. Patrol dispatched.'}], submittedAt:'29 Mar 2026, 11:05 PM' },
  { id:'ST-2904', type:'Road Hazard', location:'Adyar Signal Junction', date:'2026-03-29', time:'08:30', description:'Large pothole on the left lane causing vehicles to swerve dangerously into oncoming traffic.', severity:'medium', status:'resolved', statusHistory:[{status:'submitted',ts:'29 Mar 2026, 8:34 AM',note:''},{status:'reviewing',ts:'29 Mar 2026, 9:00 AM',note:''},{status:'resolved',ts:'29 Mar 2026, 3:15 PM',note:'BBMP notified. Pothole marked and cordoned off.'}], submittedAt:'29 Mar 2026, 8:34 AM' },
];

(function seedIfEmpty() { if (readStore().length === 0) writeStore(DEMO); })();

function useComplaints() {
  const [list, setList] = useState(readStore);
  useEffect(() => {
    const sync = () => setList(readStore());
    const iv = setInterval(sync, 1500);
    let bc;
    try { bc = new BroadcastChannel(BC_NAME); bc.onmessage = sync; } catch {}
    window.addEventListener('storage', sync);
    return () => { clearInterval(iv); window.removeEventListener('storage', sync); try { bc?.close(); } catch {} };
  }, []);
  ...
  const updateStatus = (id, newStatus, note = '') => {
    ...
    if (!c) return;
    c.status = newStatus;
    c.statusHistory.push({ status: newStatus, ts: new Date().toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' }), note });
    writeStore(data);
    setList([...data]);
  };
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 1000, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', animation: 'overlayIn .2s ease' }}>
      ...
      <div style={{
        ...
        maxHeight: '92vh', width: '100%', maxWidth: 393,
...
export default function PoliceDashboard() {
  const { list, updateStatus } = useComplaints();
  const [dark,     setDark]     = useState(() => localStorage.getItem('st-police-theme') !== 'light');
  const [filter,   setFilter]   = useState('all');
  const [search,   setSearch]   = useState('');
  const [selected, setSelected] = useState(null);
  const [seenIds,  setSeenIds]  = useState(() => new Set(readStore().map(c => c.id)));
  ...
  useEffect(() => {
    if (list.length > prevLen.current) {
      setTimeout(() => setSeenIds(new Set(readStore().map(c => c.id))), 8000);
    }
  ...
  return (
    <div style={{ height: '100vh', width: '100vw', background: t.bgDeep, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', transition: 'background .3s' }}>
      <style>{CSS}</style>
